...
router.post('/add-career', async (req, res) => {
    try {
        const { userName, userEmail, userContact, expectedSalary, location, jobTitle, skills ,resumeFile} = req.body;
        ...
        res.status(200).json({ message: 'Career added successfully', newCareer });
    } catch (error) {
        console.error(error);
